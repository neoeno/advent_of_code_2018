...
def find_sleepiest_minute(days):
    ...
    return max(range(0, 59), key=times_sleeping)

def sleepiest_guard_minute(events):
    days = calculate_days(events)
    days_by_guards = calculate_days_by_guards(days)
    sleepiest_guard = find_sleepiest_guard(days_by_guards)
    sleepiest_minute = find_sleepiest_minute(days_by_guards[sleepiest_guard])
