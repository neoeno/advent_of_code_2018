...
class Event(NamedTuple):
    ...
    def date_id(self):
        return self.time.date().strftime("%Y-%m-%d")
...
class BeginEvent(NamedTuple):
    ...
    def date_id(self):
        if self.time.hour == 0:
            ...
            next_day = self.time.date() + timedelta(days=1)
...
class Day(NamedTuple):
    ...
    def minutes_asleep(self):
        return sum(rest.stop - rest.start for rest in self.rests)

def calculate_days(events):
    days = dict()
    sorted_events = sort_events(events)
    for key, event in enumerate(sorted_events):
        if event.type == EventType.BEGIN:
            days[event.date_id()] = Day(event.guard, [])
        elif event.type == EventType.WAKE:
            rest = range(sorted_events[key - 1].time.minute, event.time.minute)
            days[event.date_id()].rests.append(rest)
    return days.values()

def calculate_days_by_guards(days):
    guards = defaultdict(list)
    for day in days:
        guards[day.guard].append(day)
    return guards

def calculate_minutes_asleep_by_guard(days_by_guards):
    guards = defaultdict(int)
    for guard, days in days_by_guards.items():
        guards[guard] = sum(day.minutes_asleep() for day in days)
    return guards

def find_sleepiest_guard(days_by_guards):
    minutes_asleep_by_guard = calculate_minutes_asleep_by_guard(days_by_guards)
    return max(minutes_asleep_by_guard, key=minutes_asleep_by_guard.get)

def rests_for_days(days):
    for day in days:
        yield from day.rests

def find_sleepiest_minute(days):
    def times_sleeping(minute):
        return sum(1 for rest in rests_for_days(days) if minute in rest)
    sleepiest_minute = max(range(0, 59), key=times_sleeping)
    return (sleepiest_minute, times_sleeping(sleepiest_minute))

def sleepiest_guard_minute(events):
    days = calculate_days(events)
    days_by_guards = calculate_days_by_guards(days)
    sleepiest_minutes = sleepiest_minute_for_guards(days_by_guards)
    sleepiest_guard = max(sleepiest_minutes, key=lambda guard: sleepiest_minutes[guard][1])
    return (sleepiest_guard, sleepiest_minutes[sleepiest_guard][0])

def sleepiest_minute_for_guards(days_by_guards):
    guards = dict()
    for guard, days in days_by_guards.items():
        guards[guard] = find_sleepiest_minute(days_by_guards[guard])
    return guards


print(sleepiest_guard_minute(list(parse_events(
